...
void do_prompt(CHAR_DATA * ch, char *argument)
{
    if (!IS_NPC(ch))
    {
        if (!strcmp(argument, "default"))
        {
            ...
            ch->pcdata->prompt =
                str_dup
                ("%i`K/`W%H`w HP %n`K/`W%M`w MP %w`K/`W%V`w MV `K> ");
        }
...
void do_afk(CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char
        ("You are now set AFK.  Messages are being recorded.\n\r", ch);
                          ...
                          int depth)
{
    ...
    for (i = 1; (i <= depth && (pexit = cur_room->exit[move_dir])
                 && pexit->u1.to_room && pexit->u1.to_room != cur_room)
    ...
    if (!strcasecmp(arg, "i") || !strcasecmp(arg, "in"))
    {
        ...
        case ITEM_PORTAL:
            ...
            if ((pRoom = get_room_index(obj->value[0])) == NULL
                    || obj->value[0] == 0)
            {
                send_to_char("You can't see anything but a hazy mist.",
                             ch);
                return;
            }
...
void do_comment(CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char
        ("Your comment cannot be longer than 60 characters.\n\r", ch);
        return;
    }
...
void do_email(CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char
        ("Email address can't be longer than 45 characters.\n\r", ch);
        return;
    }
...
void do_report(CHAR_DATA * ch, char *argument)
{
    ...
    sprintf(buf,
            "`B$n says 'I have %d/%d hp %d/%d mana %d/%d mv %ld xp.'`w",
...
void do_finger(CHAR_DATA * ch, char *argument)
{
    ...
    if (victim != NULL)
    {
        ...
        strcat(buf, "|\n\r");
        send_to_char(buf, ch);
        sprintf(buf, "     | `GIncarnations:`Y %d`y",
                victim->incarnations);
        send_to_char(buf, ch);
        x = str_len(buf);
        strcpy(buf, "\0");
    ...
    else
    {
        ...
        if ((fp = fopen(pfile, "r")) != NULL)
        {
            ...
            send_to_char(buf, ch);
            sprintf(buf,
                    "     | `GPK kills : `Y%-5d `WLast killed by: %s`y",
                    pk_kills, nemesis);
            send_to_char(buf, ch);
            x = str_len(buf);
            strcpy(buf, "\0");
...
void do_levelgain(CHAR_DATA * ch, char *argument)
{
    ...
    if (ch->level < LEVEL_HERO && ch->exp >=
            exp_per_level(ch, ch->pcdata->points))
    {
        sprintf(buf, "%s has made it to level %d!", ch->name,
                ch->level + 1);
        log_string(buf);
        do_sendinfo(ch, buf);
