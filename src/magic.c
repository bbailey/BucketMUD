        ...
        if (vch->in_room == ch->in_room)
        {
            if (vch != ch && !is_safe_spell(ch, vch, TRUE))
            {
                if (IS_AFFECTED(vch, AFF_FLYING))
                    ...
                    damage(ch, vch, NULL, level + dice(2, 8), sn,
                           DAM_BASH);
            }
            continue;
        }
...
void spell_nexus(int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    act("$n waves his hands and a portal appears out of thin air.", ch,
        NULL, NULL, TO_ROOM);
    send_to_char
...
void spell_lightning_bolt(int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    if (saves_spell(level, victim))
        dam /= 2;
    act("$n directs a bolt of lightning at $N.", ch, NULL, victim,
        TO_ROOM);
    damage(ch, victim, NULL, dam, sn, DAM_LIGHTNING);
    return;
}
...
void spell_magic_missile(int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    if (saves_spell(level, victim))
        dam /= 2;
    act("A green glowing arrow fires from $n's outstretched hand.", ch,
        NULL, NULL, TO_ROOM);
...
void spell_remove_curse(int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    for (iWear = 0; (iWear < MAX_WEAR && !found); iWear++)
    {
        ...
        if (IS_OBJ_STAT(obj, ITEM_NODROP)
                || IS_OBJ_STAT(obj, ITEM_NOREMOVE))  	/* attempt to remove curse */
        {
...
void spell_acid_breath(int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    {
        for (obj_lose = ch->carrying; obj_lose != NULL;
                obj_lose = obj_next)
        {
            int iWear;
            ...
            case ITEM_CONTAINER:
                act("$p fumes and dissolves, destroying some of the contents.", victim, obj_lose, NULL, TO_CHAR);
                /* save some of  the contents */

                for (t_obj = obj_lose->contains; t_obj != NULL;
                        t_obj = n_obj)
                {
...
void spell_ice_ray(int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    act("A blue ray from $n's hand strikes $N.", ch, NULL, victim,
        TO_NOTVICT);
    act("A blue ray shoots from your finger and strikes $N.", ch, NULL,
        victim, TO_CHAR);
    act("$n points at you sending a chilling ray into your chest.", ch,
        NULL, victim, TO_VICT);
    ...
    while (num_blast > 0)
    {
                ...
                tmp_vict != NULL; tmp_vict = next_vict)
        {
                    ...
                    && num_blast > 0)
            {
                ...
                act("A jet of flames flies from $n's hand and engulfs you!", ch, NULL, tmp_vict, TO_VICT);
                ...
                tmp_vict != NULL; tmp_vict = next_vict)
        {
                    ...
                    && num_blast > 0)
            {
                ...
                act("A blue ray shoots from your finger and strikes $N.",
                    ch, NULL, tmp_vict, TO_CHAR);
                act("$n points at you sending a chilling ray into your chest.", ch, NULL, tmp_vict, TO_VICT);
...
void spell_regeneration(int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    if (IS_AFFECTED(victim, AFF_REGENERATION))
    {
        if (victim == ch)
            send_to_char
            ("Your body is already in a regenerative state.\n\r", ch);
        else
    ...
    if (ch != victim)
        act("You imbue $N's body with regenerative powers.", ch, NULL,
            victim, TO_CHAR);
    return;
}
...
void spell_remove_align(int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    if (IS_OBJ_STAT(obj, ITEM_ANTI_EVIL)
            ...
            || IS_OBJ_STAT(obj, ITEM_ANTI_GOOD))  	/* attempt to remove curse */
    {
        if (!saves_dispel(level, obj->level, 0))
        {
