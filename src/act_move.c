...
void move_char(CHAR_DATA * ch, int door, bool follow)
{
    ...
    if (IS_AFFECTED(ch, AFF_WEB))
    {
        ...
        else
        {
            ...
            act("$n struggles weakly in the webs.", ch, NULL, NULL,
                TO_ROOM);
            WAIT_STATE(ch, 5);
            ch->move -= 5;
            return;
    ...
    if (!IS_NPC(ch))
    {
        ...
        if (ch->pcdata->condition[COND_DRUNK] > 10)
        {
            ...
            {
                act("You feel a little drunk.. not to mention kind of lost..", ch, NULL, NULL, TO_CHAR);
                act("$n looks a little drunk.. not to mention kind of lost..", ch, NULL, NULL, TO_ROOM);
                door = number_range(0, 5);
            }
            else
            {
                act("You feel a little.. drunk..", ch, NULL, NULL,
                    TO_CHAR);
                act("$n looks a little.. drunk..", ch, NULL, NULL,
                    TO_ROOM);
            }
        }
    }
    ...
    if (IS_SET(pexit->exit_info, EX_CLOSED)
            ...
            && IS_AFFECTED(ch, AFF_PASS_DOOR))
    {
        act("A magical force prevents you from passing through the $d.",
...
void do_sleep(CHAR_DATA * ch, char *argument)
{
    ...
    case POS_STANDING:
        ...
        {
            send_to_char
            ("You go to sleep.  Messages are being recorded.\n\r", ch);
...
void do_track(CHAR_DATA * ch, char *argument)
{
    ...
    default:
...
#ifdef TRACK_IS_SKILL
        {
            ...
            {
                sprintf(buf,
                        "You can't sense a trail to %s from here.\n\r",
                        PERS(vict, ch));
                send_to_char(buf, ch);
                return;
            }
