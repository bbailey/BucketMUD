    ...
    if (CAN_WEAR(obj, ITEM_WEAR_NECK))
    {
        ...
        if (get_eq_char(ch, WEAR_NECK_1) == NULL)
        {
            if (!oprog_use_trigger(ch, obj, NULL, NULL))
            {
                ...
                act("You wear $p around your neck.", ch, obj, NULL,
                    TO_CHAR);
            }
    ...
    else  			/* object purchase code begins HERE */
    {
        ...
        if (ch->fighting)
        {
            if (shopkeeper_kick_ch
                    (ch, keeper,
                     "$t kicks you out of the store for fighting!"))
            {
                do_look(ch, "");
            }
            else
    ...
    else
    {
        ...
        for (obj = ch->carrying; obj != NULL; obj = obj_next)
        {
                    ...
                    && obj->wear_loc == WEAR_NONE && can_drop_obj(ch, obj))
            {
                ...
                ch->gold += gold;
                act("$n sacrifices $p to the gods.", ch, obj, NULL,
                    TO_ROOM);
                extract_obj(obj);
            }
        }
...
bool shopkeeper_kick_ch(CHAR_DATA * ch, CHAR_DATA * keeper, char *actstr)
{
    ...
    for (attempt = 0; attempt != 6; attempt++)
    {
        ...
        act_new(actstr, ch, keeper->short_descr, NULL, TO_CHAR,
                POS_RESTING);
        act_new("$t kicks you out of the store!", ch, keeper->short_descr,
                NULL, TO_CHAR, POS_RESTING);
        act_new("$n is kicked out of the store!", ch, NULL, NULL,
                    ...
                    tmpch = tmpch->next)
            {
                if (tmpch->fighting)
                {
                    act_new("$t kicks you out of the store!", tmpch,
                            keeper->short_descr, NULL, TO_CHAR,
                            POS_RESTING);
