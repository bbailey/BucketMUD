            ...
            if (pMobIndex->area == pArea && pMobIndex->progtypes)  	/* prog */
            {
                ...
                for (pGroupList = pMobIndex->mprog_groups; pGroupList;
                    ...
                    fprintf(fp, "M %d %d %s Load to: %s\n",
                            pMobIndex->vnum, pGroupList->mprog_group->vnum,
                ...
                for (pList = pMobIndex->mudprogs; pList;
                        pList = pList->next)
                {
                    iFound = 0;
                    ...
                    for (pGroupList = pMobIndex->mprog_groups; pGroupList;
                            pGroupList = pGroupList->next)
                        for (pInnerList =
                                    pGroupList->mprog_group->mudprogs; pInnerList;
                    ...
                    case 'P':
                        ...
                        fprintf(fp, "P 0 %d 0 %d %s put inside %s\n",
                                ...
                                capitalize(get_obj_index
                                           (pReset->vnum)->short_descr),
                                pLastObj ? pLastObj->short_descr :
                                "!NULL OBJ!");
                        if (!pLastObj)
                        {
                            bug("Save_resets: !NO_OBJ! in [%s]",
...
void save_mudprogs(void)
{
    ...
    *buf = '\0';
    ...
    {
        fprintf(fp, "#%d\n%s~\n%s~\n", pMprogGroup->vnum,
                pMprogGroup->name, pMprogGroup->description);
