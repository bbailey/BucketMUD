...
static const char *color_mtoi(const char *txt)
{
    ...
    imcstrlcpy(tbuf, txt, LGST);
    for (color = first_imc_color; color; color = color->next)
        imcstrlcpy(tbuf, imcstrrep(tbuf, color->mudtag, color->imctag),
                   LGST);
...
static int get_imcpermvalue(const char *flag)
{
    ...
    for (x = 0; x < (sizeof(imcperm_names) / sizeof(imcperm_names[0]));
            x++)
                       ...
                       bool all)
{
    ...
    if (all)
    {
        for (c = first_imc_channel; c; c = c->next)
        {
            ...
            {
                snprintf(buf, LGST, "~R[~Y%s~R] ~C%%s: ~c%%s",
                         c->local_name);
                IMCSTRFREE(c->regformat);
...
static void update_imchistory(IMC_CHANNEL * channel, char *message)
{
    ...
    for (x = 0; x < MAX_IMCHISTORY; x++)
    {
        if (channel->history[x] == NULL)
        {
            ...
            {
                FILE *fp;
                snprintf(buf, LGST, "%s%s.log", IMC_DIR,
                         channel->local_name);
                if (!(fp = fopen(buf, "a")))
                {
                    perror(buf);
                         ...
                         int emote)
{
    ...
    if (emote < 2)
        snprintf(buf, LGST, emote ? c->emoteformat : c->regformat, from,
                 txt);
    else
    ...
    for (d = first_descriptor; d; d = d->next)
    {
        ...
        if (!c->open)
        {
            ...
            if (!imc_hasname(c->invited, name)
                    && strcasecmp(c->owner, name))
                continue;
        }
        imc_printf(ch, "%s\r\n", buf);
    }
                           ...
                           char *immheader)
{
    ...
    imcstrlcpy(master, imcstrrep(master, "<%plrheader%>", plrheader),
               LGST);
    imcstrlcpy(master, imcstrrep(master, "<%immheader%>", immheader),
               LGST);
...
static char *imc_assemble_who(void)
{
    ...
    char plrheader[SMST], immheader[SMST], rank[SMST], flags[SMST],
    name[SMST], title[SMST], plrline[SMST], immline[SMST];
    ...
    imcstrlcpy(master,
               process_who_template(head, tail, plrlines, immlines,
                                    plrheader, immheader), LGST);
...
static void imc_process_finger(const char *from, const char *type)
{
    ...
    snprintf(buf, IMC_BUFF_SIZE,
             ...
             CH_IMCNAME(victim),
             (IMCAFK(victim) ? "AFK" : "Lurking about"),
             imcperm_names[IMCPERM(victim)], (IMC_LISTEN(victim)
                                              && IMC_LISTEN(victim)[0] !=
                                              '\0') ? IMC_LISTEN(victim) :
             "None");
    ...
    if (!IMCIS_SET(IMCFLAG(victim), IMC_PRIVACY))
        snprintf(buf + strlen(buf), IMC_BUFF_SIZE - strlen(buf),
                 ...
                 (IMC_AIM(victim)
                  && IMC_AIM(victim)[0] !=
                  '\0') ? IMC_AIM(victim) : "None", (IMC_YAHOO(victim)
                                                     &&
                                                     IMC_YAHOO(victim)[0]
                                                     !=
                                                     '\0') ?
                 IMC_YAHOO(victim) : "None", (IMC_MSN(victim)
                                              && IMC_MSN(victim)[0] !=
                                              '\0') ? IMC_MSN(victim) :
                 "None");
    ...
    snprintf(buf + strlen(buf), IMC_BUFF_SIZE - strlen(buf), "~W%s\r\n",
             (IMC_COMMENT(victim)
              && IMC_COMMENT(victim)[0] !=
              '\0') ? IMC_COMMENT(victim) : "");
...
static PFUN(imc_recv_isalive)
{
    ...
    {
        imc_new_reminfo(imc_mudof(q->from), version, netname, url,
                        q->route);
        return;
    }
...
static PFUN(imc_recv_iceupdate)
{
    ...
    if (!(c = imc_findchannel(chan)))
    {
        imc_new_channel(chan, owner, ops, invite, exclude, copen, perm,
                        lname);
        return;
    }
...
static void imc_register_default_packets(void)
{
    ...
    imc_register_packet_handler("ice-chan-whoreply",
                                imc_recv_chanwhoreply);
...
static void imc_parse_packet(const char *packet)
{
    ...
    if (!(imc_find_reminfo(imc_mudof(p->from))))
        imc_new_reminfo(imc_mudof(p->from), "Unknown",
                        this_imcmud->network, "Unknown", p->route);
                          ...
                          char *txt, char *encrypt)
{
    if (!strcasecmp(cmd, "reject"))
    {
        ...
        {
            imclog
            ("There is already a mud named %s connected to the network.",
             this_imcmud->localname);
            imc_shutdown(FALSE);
            return;
        }
        ...
        {
            imclog("%s is a private server. Autosetup denied.",
                   servername);
            imc_shutdown(FALSE);
            return;
        }
        ...
        {
            imclog
            ("%s has reached its connection limit. Autosetup denied.",
             servername);
            imc_shutdown(FALSE);
            return;
        }
        ...
        imclog("%s: Invalid 'reject' response. Autosetup failed.",
               servername);
        imclog("Data received: %s %s %s %s %s", source, servername, cmd,
               txt, encrypt);
        imc_shutdown(FALSE);
        return;
    }
...
static void imc_process_authentication(const char *packet)
{
    ...
    {
        imclog("%s",
               "Incomplete authentication packet. Unable to connect.");
        imc_shutdown(FALSE);
        return;
    }
...
static bool imc_read_socket(void)
{
    ...
    for (;;)
    {
        ...
        else if (nRead == 0 && this_imcmud->state == IMC_ONLINE)
        {
            ...
            imclog("%s",
                   "Connection close detected on read of IMC2 socket.");
            return FALSE;
        }
...
void imc_loop(void)
{
    ...
    if (select(this_imcmud->desc + 1, &in_set, &out_set, NULL, &null_time)
            < 0)
    {
...
static void imc_loadhistory(void)
{
    ...
    for (tempchan = first_imc_channel; tempchan; tempchan = tempchan->next)
    {
        ...
        snprintf(filename, 256, "%s%s.hist", IMC_DIR,
                 tempchan->local_name);
...
static void imc_loadchannels(void)
{
    ...
    for (;;)
    {
        ...
        if (!strcasecmp(word, "IMCCHAN"))
        {
            ...
            IMCLINK(channel, first_imc_channel, last_imc_channel, next,
                    prev);
    ...
    for (;;)
    {
        ...
        case 'A':
            if (!strcasecmp(word, "Alias"))
            {
                ...
                IMCLINK(alias, cmd->first_alias, cmd->last_alias, next,
                        prev);
                fMatch = TRUE;
                break;
            }
...
static void imcfread_config_file(FILE * fin)
{
    ...
    for (;;)
    {
        ...
        case 'I':
            IMCKEY("Implevel", this_imcmud->implevel,
                   imcfread_number(fin));
            ...
            IMCKEY("InfoDetails", this_imcmud->details,
                   imcfread_line(fin));
            break;

        case 'L':
            IMCKEY("LocalName", this_imcmud->localname,
                   imcfread_line(fin));
            break;
...
static bool imc_read_config(int desc)
{
    ...
    for (;;)
    {
        ...
        {
            imcbug("imc_read_config: Bad section in config file: %s",
                   word);
            continue;
        }
    }
    ...
    {
        imcbug("%s",
               "imc_read_config: No server connection information!!");
        imcbug("%s", "Network configuration aborted.");
        return FALSE;
    }

    if (!this_imcmud->rhost || !this_imcmud->clientpw
            || !this_imcmud->serverpw)
    {
        imcbug("%s",
    ...
    {
        snprintf(cbase, SMST, "%s%s", IMC_VERSION_STRING,
                 this_imcmud->base);
...
static char *parse_who_header(char *head)
{
    ...
    imcstrlcpy(newhead,
               imcstrrep(newhead, "<%mudfullname%>",
                         this_imcmud->fullname), LGST);
    imcstrlcpy(newhead,
               imcstrrep(newhead, "<%mudtelnet%>", this_imcmud->ihost),
               LGST);
...
static char *parse_who_tail(char *tail)
{
    ...
    imcstrlcpy(newtail,
               imcstrrep(newtail, "<%mudfullname%>",
                         this_imcmud->fullname), LGST);
    imcstrlcpy(newtail,
               imcstrrep(newtail, "<%mudtelnet%>", this_imcmud->ihost),
               LGST);
...
static void imc_load_who_template(void)
{
    ...
    {
        imclog("%s: Unable to load template file for imcwho",
               __FUNCTION__);
        whot = NULL;
        return;
    }
...
static int ipv4_connect(void)
{
    ...
    if (!inet_aton(this_imcmud->rhost, &sa.sin_addr))
    {
        ...
        {
            imclog("%s",
                   "imc_connect_to: Cannot resolve server hostname.");
            imc_shutdown(FALSE);
            return -1;
        }
...
static bool imc_server_connect(void)
{
    ...
    if (ai == NULL)
    {
        imclog("%s: socket or connect: failed for %s port %hu",
               __FUNCTION__, this_imcmud->rhost, this_imcmud->rport);
    ...
    if (this_imcmud->sha256)
    {
         ...
         */
            ...
            snprintf(buf, LGST, "SHA256-AUTH-REQ %s",
                     this_imcmud->localname);
    }
    /*
...
static void free_imcdata(bool complete)
{
    ...
    if (complete)
    {
        ...
        for (cmd = first_imc_command; cmd; cmd = cmd_next)
        {
            ...
            IMCSTRFREE(cmd->name);
            IMCUNLINK(cmd, first_imc_command, last_imc_command, next,
                      prev);
            IMCDISPOSE(cmd);
        }
...
static IMC_CMD(imcsetup)
{
    ...
    if (!strcasecmp(imccmd, "resetformats"))
    {
        if (all)
        {
            ...
            imc_to_char
            ("All channel formats have been reset to default.\r\n",
             ch);
        }
        else
        {
    ...
    if (!strcasecmp(imccmd, "regformat"))
    {
        ...
        if (all)
        {
            ...
            imc_to_char
            ("All channel regular formats have been updated.\r\n", ch);
        }
        else
        {
            IMCSTRFREE(c->regformat);
    ...
    if (!strcasecmp(imccmd, "socformat"))
    {
        ...
        if (all)
        {
            ...
            imc_to_char
            ("All channel social formats have been updated.\r\n", ch);
        }
        else
        {
            IMCSTRFREE(c->socformat);
            ...
            || !strcasecmp(imccmd, "level"))
    {
        ...
        {
            imc_to_char
            ("You cannot do a permissions all for channels.\r\n", ch);
            return;
        }
...
static IMC_CMD(imclist)
{
    ...
    for (p = first_rinfo; p; p = p->next, count++)
    {
        ...
        imcpager_printf(ch, "\r\n~%c%-15.15s ~B%-40.40s ~g%-15.15s ~G%s",
                        p->expired ? 'R' : 'c', p->name, p->version,
                        netname, serverpath);
    }
    imcpager_printf(ch,
...
static IMC_CMD(imcconfig)
{
    ...
    if (arg1[0] == '\0')
    {
        ...
        ("~wInfoemail      : ~GEmail address of the mud's IMC administrator.\r\n",
         ch);
        imc_to_char
        ("~wInfoWWW        : ~GThe Web address of your mud.\r\n", ch);
        imc_to_char("~wInfoBase       : ~GThe codebase your mud uses.\r\n",
                    ch);
        imc_to_char
        ("~wInfoDetails    : ~GSHORT Description of your mud.\r\n",
         ch);
        imc_to_char
        ...
        ("~wServerPort     : ~GPort of the server your mud connects to.\r\n",
         ch);
        imc_to_char
        ("~wClientPwd      : ~GClient password for your mud.\r\n", ch);
        imc_to_char
        ("~wServerPwd      : ~GServer password for your mud.\r\n", ch);
        return;
    }
...
static IMC_CMD(imcignore)
{
    ...
    if (!strcasecmp(arg, "delete"))
    {
        for (ign = FIRST_IMCIGNORE(ch); ign; ign = ign->next)
        {
            ...
            {
                IMCUNLINK(ign, FIRST_IMCIGNORE(ch), LAST_IMCIGNORE(ch),
                          next, prev);
                IMCSTRFREE(ign->name);
                IMCDISPOSE(ign);
...
static IMC_CMD(imchelp)
{
    ...
    {
        imcstrlcpy(buf,
                   "~gHelp is available for the following commands:\r\n",
                   LGST);
        imcstrlcat(buf,
                   "~G---------------------------------------------\r\n",
                   LGST);
    ...
    for (help = first_imc_help; help; help = help->next)
    {
        if (!strcasecmp(help->name, argument))
        {
            if (!help->text || help->text[0] == '\0')
                imc_printf(ch,
                           "~gNo inforation available for topic ~W%s~g.\r\n",
                           help->name);
            else
                imc_printf(ch, "~g%s\r\n", help->text);
            return;
    ...
    if (!strcasecmp(option, "alias"))
    {
        ...
        for (tmp = first_imc_command; tmp; tmp = tmp->next)
        {
            ...
            for (alias = tmp->first_alias; alias; alias = alias->next)
            {
                ...
                {
                    imc_printf(ch,
                               "~W%s ~gis already an alias for ~W%s\r\n",
                               argument, tmp->name);
                    return;
                }
            }
...
static IMC_CMD(imclast)
{
    ...
    p = imc_newpacket(CH_IMCNAME(ch), "imc-laston",
                      this_imcmud->servername);
    if (argument && argument[0] != '\0')
                            ...
                            int victim)
{
    ...
    {
        if (person && person[0] != '\0'
                && !strcasecmp(person, CH_IMCNAME(ch))
                && mud && mud[0] != '\0'
        ...
        else
        {
            ...
            else if (victim == 1)
            {
                ...
                {
                    imc_printf(ch,
                               "~YSocial ~W%s~Y: Missing vict_found.\r\n",
                               sname);
                    return socname;
                }
            ...
            else
            {
                ...
                {
                    imc_printf(ch,
                               "~YSocial ~W%s~Y: Missing char_found.\r\n",
                               sname);
                    return socname;
                }
    ...
    else
    {
        if (victim == 0 || victim == 1)
        {
            ...
            {
                imc_printf(ch,
                           "~YSocial ~W%s~Y: Missing others_no_arg.\r\n",
                           sname);
                return socname;
            }
...
static char *imc_act_string(const char *format, CHAR_DATA * ch, CHAR_DATA * vic)
{
    ...
    while (*format != '\0')
    {
        if (*format == '.' || *format == '?' || *format == '!')
            should_upper = TRUE;
        else if (should_upper == TRUE && !isspace(*format)
                 && *format != '$')
            should_upper = FALSE;
                            ...
                            int telloption)
{
    ...
    if (argument && argument[0] != '\0')
    {
        ...
        {
            imc_to_char
            ("You need to specify a person@mud for a target.\r\n", ch);
            return "";
        }
        else
        {
                      ...
                      const char *argument)
{
    ...
    for (cmd = first_imc_command; cmd; cmd = cmd->next)
    {
        ...
        if (!strcasecmp(command, cmd->name))
        {
            ...
            {
                imc_to_char
                ("The mud is not currently connected to IMC2.\r\n",
                 ch);
                return TRUE;
            }
