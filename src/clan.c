                        ...
                        bool is_petitioning)
{
    ...
    {
        send_to_char
        ("You may not be a player killer and join this clan.\n\r", ch);
        send_to_char
...
void do_join(CHAR_DATA * ch, char *argument)
{
    ...
    case JOIN_DECIDING:
        send_to_char("You have already been invited to join a clan.\n\r",
                     ch);
        send_to_char
        ("Use the ACCEPT or DECLINE commands to join or not.\n\r", ch);
        return;
...
void do_petition(CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char
        ("You have already been offered membership in a clan!\n\r",
         ch);
    ...
    printf_to_char(victim, "%s has petitioned to join your clan!\n\r",
                   ch->name);
    send_to_char
    ("Type ACCEPT or DECLINE, or talk to them for a while.\n\r",
     victim);
...
void do_offer(CHAR_DATA * ch, char *argument)
{
    ...
    if (ch->pcdata->clan_rank < 1
            || ch->pcdata->clan_rank > RANK_CAN_ACCEPT)
    {
        send_to_char
    ...
    if (victim->pcdata->clan != 0)
    {
        ...
        if (IS_SET(tmpclan->clan_flags, CLAN_SECRET))
            send_to_char
            ("That person is ineligable to join your clan.\n\r", ch);
        else
    ...
    if (clan->cost_gold > 0)
        printf_to_char(victim, "It will cost %d gold to join.\n\r",
                       clan->cost_gold);
    clan_log(clan, "Membership offered to %s by %s.", victim->name,
             ch->name);
    ch->pcdata->clan_ch = victim;
...
void do_promote(CHAR_DATA * ch, char *argument)
{
    ...
    if (get_trust(ch) < MAX_LEVEL)  	/* IMPS can promote anyone :) */
    {
        ...
        {
            send_to_char
            ("You are not high enough rank to promote anyone.\n\r",
             ch);
            return;
        }
...
void do_clan(CHAR_DATA * ch, char *argument)
{
    ...
    if (!strcasecmp(arg1, "list"))
    {
        ...
        {
            send_to_char
            ("There are no clans.  Use 'edit clan create'.\n\r", ch);
            return;
        }
        send_to_char
    ...
    if (!strcasecmp(arg2, "member"))
    {
        ...
        if (!strcasecmp(arg3, "add"))
        {
            ...
            printf_to_char(victim,
                           "You have been made a member of %s.\n\r",
                           clan->name);
    ...
    if (!strcasecmp(arg2, "status"))
    {
        if (!strcasecmp(arg3, "creating"))
        {
            ...
            SET_BIT(clan->clan_flags, CLAN_CREATING);
            printf_to_char(ch,
                           "Clan %s is now in the 'creating' stage.\n\r",
                           clan->name);
            save_clans();
            return;
        }
        ...
        if (!strcasecmp(arg3, "inactive"))
        {
            ...
            printf_to_char(ch,
                           "Clan %s is now in the 'inactive' stage.\n\r",
                           clan->name);
        ...
        if (!strcasecmp(arg3, "disbanding"))
        {
            ...
            printf_to_char(ch,
                           "Clan %s is now in the 'disbanding' stage.\n\r",
                           clan->name);
...
void do_cdeposit(CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char
        ("The clan deposit command is for clan members only.\n\r", ch);
        return;
    }
...
void do_crecall(CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char
        ("The clan recall command is for clan members only.\n\r", ch);
        return;
    }
    ...
    if (!IS_NPC(ch))
    {
        ...
        {
            send_to_char
            ("You can't use clan recall, you're not in a clan!", ch);
            return;
        }
        clan = get_clan(ch->pcdata->clan);
...
void do_demote(CHAR_DATA * ch, char *argument)
{
    ...
    if (get_trust(ch) < MAX_LEVEL)  	/* IMPS can demote anyone :) */
    {
        ...
        {
            send_to_char
            ("You are not high enough rank to demote anyone.\n\r", ch);
            return;
        }
