...
void do_award(CHAR_DATA * ch, char *argument)
{
    ...
    if (arg1[0] == '\0' || arg3[0] == '\0' || !is_number(arg2))
    {
        ...
        send_to_char
        ("Valid types: alignment, experience, gold, object.\n\r", ch);
        send_to_char
        ("NOTE: Substitute the object's VNUM for the amount.\n\r", ch);
        return;
    }
    ...
    {
        send_to_char("You can only award those of lesser trust level.",
                     ch);
        return;
    }
    ...
    if (arg3[0] == 'a')  	/* alignment */
    {
        ...
        {
            sprintf(buf,
                    "%s's alignment is already as %s as it can be.\n\r",
                    victim->name, maxed);
            send_to_char(buf, ch);
            return;
        }
...
static void recursive_clone(CHAR_DATA * ch, OBJ_DATA * obj,
                            OBJ_DATA * clone)
{
