...
static void copyover_recover(void)
{
    ...
    for (;;)
    {
        ...
        else  		/* ok! */
        {
            ...
            {
                char_to_room(dnew->character->pet,
                             dnew->character->in_room);
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (letter == 'D')
            {
                ...
                case 4:
                    pexit->rs_flags =
                        EX_ISDOOR | EX_PICKPROOF | EX_PASSPROOF |
                        EX_HIDDEN;
                    break;
                case 5:
            ...
            for (door = 0; door <= 5; door++)
            {
                            ...
                            || pRoomIndex->vnum > 1299))
                {
                    sprintf(buf, "Fix_exits: %d:%d -> %d:%d -> %d.",
                            ...
                            rev_dir[door], (pexit_rev->u1.to_room == NULL)
        ...
        else if (!str_cmp(word, "#MOBPROGS"))
        {
            for (;;)
            {
                ...
                pMudProg->trigger_type =
                    mprog_name_to_type(fread_word(fp));
        ...
        if (!iFound)
        {
                    ...
                    pTempList = pTempList->next)
            {
                ...
                for (pInnerList = pMob->mudprogs; pInnerList;
                    ...
                    if (pInnerList->mudprog->vnum ==
                            pTempList->mudprog->vnum)
                        iFound = 1;
