...
static bool change_exit(CHAR_DATA * ch, char *argument, int door)
{
    ...
    if (!str_cmp(command, "exit"))
    {
        if ((value = flag_value(exit_flags, arg)) != NO_FLAG)
        {
            ...
            {
                send_to_char
                ("REdit:  Cannot flag a non-existant exit.\n\r", ch);
                return FALSE;
            }
...
MPEDIT(mpedit_name)
{
    ...
    {
        send_to_char
        ("MudProg name cannot be longer than 20 characters.\n\r", ch);
        return FALSE;
    }
        ...
        if (pMudProg->vnum)
        {
            ...
            case MOB_PROG:
                ...
                for (vnum = 0; nMatch < top_mob_index; vnum++)
                    ...
                    {
                        if (IS_SET
                                (pMob->progtypes, pMudProg->trigger_type))
                            for (pList = pMob->mudprogs; pList;
                                    pList = pList->next)
                                if (pList->mudprog->vnum == pMudProg->vnum)
                                {
            ...
            case OBJ_PROG:
                ...
                for (vnum = 0; nMatch < top_obj_index; vnum++)
                    ...
                    {
                        if (IS_SET
                                (pObj->progtypes, pMudProg->trigger_type))
                            for (pList = pObj->mudprogs; pList;
                                    pList = pList->next)
                                if (pList->mudprog->vnum == pMudProg->vnum)
                                {
...
void show_mprog(CHAR_DATA * ch, MPROG_DATA * pMudProg)
{
    ...
    sprintf(buf,
            "[%4d] %-20s   -   ProgType: %s\n\rDesc: %s\n\r>%s %s~\n\r",
            pMudProg->vnum, pMudProg->name, buf2, pMudProg->description,
...
MPGEDIT(mpgedit_delete)
{
    ...
    case MOB_PROG:
        ...
        for (vnum = 0; nMatch < top_mob_index; vnum++)
            if ((pMob = get_mob_index(vnum)) != NULL)
            {
                for (pList = pMob->mprog_groups; pList;
                        pList = pList->next)
    ...
    case ROOM_PROG:
        ...
        for (vnum = 0; nMatch < top_room; vnum++)
            if ((pRoom = get_room_index(vnum)) != NULL)
            {
                for (pList = pRoom->mprog_groups; pList;
                        pList = pList->next)
                    if (pList->mprog_group->vnum == pMprogGroup->vnum)
                    {
                        send_to_char
    ...
    case OBJ_PROG:
        ...
        for (vnum = 0; nMatch < top_obj_index; vnum++)
            if ((pObj = get_obj_index(vnum)) != NULL)
            {
                for (pList = pObj->mprog_groups; pList;
                        pList = pList->next)
                    if (pList->mprog_group->vnum == pMprogGroup->vnum)
                    {
                        send_to_char
    ...
    {
        for (pTempGroup = mprog_group_first;
                pTempGroup->next != pMprogGroup;
...
MPGEDIT(mpgedit_remove)
{
    ...
    for (pList = pGroup->mudprogs; pList; pList = pList->next)
        if (pList->mudprog->vnum == progvnum)
        {
            ...
            {
                for (pInnerList = pGroup->mudprogs;
        ...
        for (vnum = 0; nMatch < top_mob_index; vnum++)
            if ((pMob = get_mob_index(vnum)) != NULL)
            {
                ...
                if (iFound)
                {
                    ...
                    for (pGroupList = pMob->mprog_groups; pGroupList;
                            pGroupList = pGroupList->next)
                        for (pList = pGroupList->mprog_group->mudprogs;
...
void show_mpgroup(CHAR_DATA * ch, MPROG_GROUP * pGroup)
{
    ...
    *buf = '\0';
    ...
    sprintf(buf, "** [%4d] Group - %s\n\r** Desc: %s\n\r\n\r",
            pGroup->vnum, pGroup->name, pGroup->description);
    page_to_char(buf, ch);
