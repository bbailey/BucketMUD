...
static int exec_proc(char *procname, int intarg, char *chararg)
{
    ...
    {
    case 'C':
        return (*mprog_cmd_table[cmd].mprog_fun) (mprog_get_actor
                (chararg, 'C'));
    case 'O':
        return (*mprog_cmd_table[cmd].mprog_fun) (mprog_get_actor
                (chararg, 'O'));
...
void parse_command_var(char var, char *outbuf)
{
    ...
    *buf = '\0';
    ...
    case 'N':
        if (ProgTriggeredBy)
            strcat(outbuf,
                   IS_NPC(ProgTriggeredBy) ? ProgTriggeredBy->short_descr :
                   ProgTriggeredBy->name);
    ...
    case 'T':
        if (ProgVictim)
            strcat(outbuf,
                   IS_NPC(ProgVictim) ? ProgVictim->short_descr :
                   ProgVictim->name);
    ...
    case 'X':
        ...
        if (MostEvilFighter)
            strcat(outbuf,
                   IS_NPC(MostEvilFighter) ? MostEvilFighter->short_descr :
                   MostEvilFighter->name);
