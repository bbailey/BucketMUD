...
static bool check_block(CHAR_DATA * ch, CHAR_DATA * victim);
...
static void start_hating(CHAR_DATA * ch, CHAR_DATA * victim)
{
    if (ch == victim)
        return;
                    ...
                    int dt)
{
    ...
    {
        if (ch->level >
                skill_table[gsn_blind_fighting].skill_level[ch->Class]
    ...
    else
    {
        ...
        if (damaged && (weapon != NULL && ch->fighting == victim))
        {
            ...
            if (IS_WEAPON_STAT(weapon, WEAPON_VAMPIRIC))
            {
                ...
                act("$p draws life from $n.", victim, weapon, NULL,
                    TO_ROOM);
                act("You feel $p drawing your life away.", victim, weapon,
                    NULL, TO_CHAR);
                    ...
                    && !is_safe(ch, victim))
            {
                ...
                act("$n's head is severed from $s body by $p!", victim,
                    weapon, NULL, TO_ROOM);
                sprintf(buf2,
...
static void make_pk_corpse(CHAR_DATA * ch)
{
    char buf[MAX_STRING_LENGTH];
    OBJ_DATA *corpse;
    OBJ_DATA *obj;
                        ...
                        int dt, bool immune)
{
    ...
    if (dt == TYPE_HIT)
    {
        if (dam > 0)
            ...
            {
                sprintf(buf1,
                        "`G$n `R%s`G $N for `g%d`G points of damage%c", vp,
                        dam, punct);
                sprintf(buf2,
                        "`YYou `R%s`Y $N for `R%d`Y points of damage%c",
                        vs, dam, punct);
                sprintf(buf3,
                        "`C$n `R%s`C you for `c%d`C points of damage%c",
                        vp, dam, punct);
            }
        else if (ch == victim)
        {
        ...
        {
            sprintf(buf1, "`G$n %s`G $N for `g%d`G points of damage%c`w",
                    vp, dam, punct);
            sprintf(buf2, "`YYou %s`Y $N for `R%d`Y points of damage%c`w",
                    vs, dam, punct);
            sprintf(buf3, "`C$n %s`C you for `c%d`C points of damage%c`w",
                    vp, dam, punct);
        }
    }
    else
    {
        ...
        else
        {
            ...
            logf_string("BUG: ^^ ch: %s victim: %s ", ch->name,
                        victim->name);
            dt = TYPE_HIT;
        ...
        if (immune)
        {
            if (ch == victim)
            {
                sprintf(buf1, "`B$n is unaffected by $s own %s.`w",
                        attack);
        ...
        else
        {
            if (dam > 0)
                if (ch == victim)
                {
                    sprintf(buf1, "`w$n's %s `R%s`w $m%c", attack, vp,
                            punct);
    ...
    else
    {
        ...
        else
        {
            if (dam > 0)
                ...
                {
                    sprintf(buf1, "`G$n's %s `R%s`G $N%c", attack, vp,
                            punct);
                    sprintf(buf2, "`YYour %s `R%s`Y $N%c", attack, vp,
                            punct);
            ...
            victim->level > skill_table[gsn_grip].skill_level[victim->Class])
    {
        ...
        if (number_percent() < skill)
        {
            act("$N grips and prevent you to disarm $S!", ch, NULL, victim,
                TO_CHAR);
            act("$n tries to disarm you, but you grip and escape!", ch,
                NULL, victim, TO_VICT);
...
void do_berserk(CHAR_DATA * ch, char *argument)
{
    ...
    if (number_percent() < chance)
    {
        ...
        send_to_char
        ("`RYour pulse races as you are consumned by rage!\n\r`w", ch);
        act("`W$n gets a wild look in $s eyes.`w", ch, NULL, NULL,
            TO_ROOM);
...
void do_dirt(CHAR_DATA * ch, char *argument)
{
    ...
    {
        AFFECT_DATA af;
        act("`W$n is blinded by the dirt in $s eyes!`w", victim, NULL,
            NULL, TO_ROOM);
...
void do_trip(CHAR_DATA * ch, char *argument)
{
    ...
    if (number_percent() < chance)
    {
        act("`W$n trips you and you go down!`w", ch, NULL, victim,
            TO_VICT);
        act("`WYou trip $N and $N goes down!`w", ch, NULL, victim,
            TO_CHAR);
...
void do_murder(CHAR_DATA * ch, char *argument)
{
    ...
    WAIT_STATE(ch, 1 * PULSE_VIOLENCE);
    if (IS_NPC(ch))
        sprintf(buf, "`YHelp! I am being attacked by %s!`w",
                ch->short_descr);
    else
...
void do_backstab(CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char
        ("You would probably only slice off your own finger!\n\r", ch);
        return;
    }
    if (victim == ch)
    {
...
void do_rescue(CHAR_DATA * ch, char *argument)
{
    ...
    if (IS_NPC(ch)
            && number_percent() >
            ((((abs(ch->level - RESCUE_PENALTY) + 1) / MAX_LEVEL) * 100)))
    {
        send_to_char("`BYou fail the rescue.\n\r`w", ch);
        return;
    }
...
void do_blackjack(CHAR_DATA * ch, char *argument)
{
    ...
    {
        act("`ROh yes baby! Direct hit! $N MUST be hurting now Muahaha!`w",
            ch, NULL, victim, TO_CHAR);
        act("`R*** WHAM! *** $n just nailed you in the face! There are 3 $n's now!`w", ch, NULL, victim, TO_VICT);
        act("`R$n just nailed $N square in the kisser. Now THAT musta hurt.`w", ch, NULL, victim, TO_NOTVICT);
    ...
    else if (chance >= 111)
    {
        act("`R *** SMASH!!!!! *** Hehe got'm good that time.`w", ch, NULL,
            victim, TO_CHAR);
        act("`R*** CRACK *** What in the world was that! It sure did hurt!`w", ch, NULL, victim, TO_VICT);
        act("`RMAN $n MUST be pissed! He just knocked $N out `WCOLD`R!`w",
