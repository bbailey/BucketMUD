...
static void load_board(BOARD_DATA * board)
{
    ...
    for (;;)
    {
        ...
        if (pnote->expire < current_time)
        {
            ...
            sprintf(archive_name, "%s/%s.old", sysconfig.note_dir,
                    board->short_name);
...
bool is_note_to(CHAR_DATA * ch, NOTE_DATA * note)
{
    ...
    if (IS_HERO(ch) && (is_full_name("hero", note->to_list) ||
                        is_full_name("heros", note->to_list) ||
                        is_full_name("heroes", note->to_list) ||
                        is_full_name("heroed", note->to_list)))
        return TRUE;
    ...
    if (ch->pcdata->clan >= 1)
    {
        ...
        if (is_full_name(clan->name, note->to_list))
            return TRUE;
    }
...
static void do_nwrite(CHAR_DATA * ch, char *argument)
{
    ...
    if (!ch->pcdata->in_progress->text)  	/* Are we continuing an old note or not? */
    {
        ...
        case DEF_EXCLUDE:
            sprintf(buf,
                    "The recipient of this note must NOT include: \"`W%s`w\".",
                    ch->pcdata->board->names);
...
void do_board(CHAR_DATA * ch, char *argument)
{
    ...
    if (!argument[0])  		/* show boards */
    {
        ...
        for (i = 0; i < MAX_BOARD; i++)
        {
            ...
            {
                sprintf(buf, "`W%2d`w> `G%12s`w [`%c%4d`w] `y%s`w\n\r",
        ...
        if (ch->pcdata->board->read_level > get_trust(ch))
            send_to_char
            ("You cannot read nor write notes on this board.\n\r", ch);
...
void handle_con_note_finish(DESCRIPTOR_DATA * d, char *argument)
{
    ...
    case 'r':			/* Format the text */
        ...
        ch->pcdata->in_progress->text =
            string_replace(ch->pcdata->in_progress->text, arg2, arg3);
        sprintf(buf, "'%s' replaced with '%s'.\n\r", arg2, arg3);
        send_to_char(buf, ch);
        write_to_buffer(d, szFinishPrompt, 0);
        write_to_buffer(d, "\n\r", 0);
        break;
    case 'w':			/* Format the text */
        if (ch->pcdata->in_progress->text)
        {
            ch->pcdata->in_progress->text =
                format_string(ch->pcdata->in_progress->text);
            write_to_buffer(d, "Your note has been word wrapped.\n\r", 0);
